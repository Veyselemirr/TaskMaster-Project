'use client'

import React from 'react'
import { Layout } from '@/components/layout/Layout'
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/Card'
import { Button } from '@/components/ui/Button'
import { Input } from '@/components/ui/Input'
import { Badge } from '@/components/ui/Badge'
import { 
  Search, 
  Plus, 
  User, 
  Calendar,
  Clock,
  AlertTriangle,
  CheckSquare,
  Flag,
  MoreVertical,
  Filter,
  Zap,
  ExclamationTriangle
} from 'lucide-react'
import { Task, TaskStatus, TaskPriority, TaskType } from '@/types/task'
import { formatDate, formatRelativeDate } from '@/lib/utils'
import Link from 'next/link'

// Mock overdue tasks data
const mockTasks: Task[] = [
  {
    id: 2,
    title: 'Setup CI/CD pipeline',
    description: 'Configure automated deployment pipeline with GitHub Actions, Docker, and AWS integration.',
    status: TaskStatus.IN_PROGRESS,
    priority: TaskPriority.HIGH,
    type: TaskType.FEATURE,
    estimatedHours: 10,
    actualHours: 7,
    progress: 70,
    dueDate: new Date('2024-09-08'), // 3 days overdue
    assignedToId: 3,
    assignedTo: { id: 3, name: 'Mike Johnson', email: 'mike@example.com', role: 'Backend Developer' },
    projectId: 1,
    project: { id: 1, name: 'TaskMaster App', status: 'ACTIVE' },
    tags: ['devops', 'ci-cd', 'deployment'],
    createdAt: new Date('2024-09-01'),
    updatedAt: new Date('2024-09-10'),
    createdById: 1,
    createdBy: { id: 1, name: 'Admin', email: 'admin@example.com', role: 'Full Stack Developer' }
  },
  {
    id: 4,
    title: 'Database migration scripts',
    description: 'Create and test database migration scripts for the new schema changes including user roles and permissions.',
    status: TaskStatus.TODO,
    priority: TaskPriority.CRITICAL,
    type: TaskType.TASK,
    estimatedHours: 6,
    actualHours: 0,
    progress: 0,
    dueDate: new Date('2024-09-05'), // 6 days overdue
    assignedToId: 3,
    assignedTo: { id: 3, name: 'Mike Johnson', email: 'mike@example.com', role: 'Backend Developer' },
    projectId: 1,
    project: { id: 1, name: 'TaskMaster App', status: 'ACTIVE' },
    tags: ['database', 'migration', 'schema'],
    createdAt: new Date('2024-08-30'),
    updatedAt: new Date('2024-09-03'),
    createdById: 1,
    createdBy: { id: 1, name: 'Admin', email: 'admin@example.com', role: 'Full Stack Developer' }
  },
  {
    id: 6,
    title: 'Fix payment integration bug',
    description: 'Resolve the critical issue with Stripe payment processing that is causing failed transactions.',
    status: TaskStatus.BLOCKED,
    priority: TaskPriority.CRITICAL,
    type: TaskType.BUG,
    estimatedHours: 4,
    actualHours: 6,
    progress: 25,
    dueDate: new Date('2024-09-07'), // 4 days overdue
    assignedToId: 2,
    assignedTo: { id: 2, name: 'John Doe', email: 'john@example.com', role: 'Lead Developer' },
    projectId: 2,
    project: { id: 2, name: 'Website Redesign', status: 'ACTIVE' },
    tags: ['payment', 'stripe', 'critical-bug'],
    createdAt: new Date('2024-09-04'),
    updatedAt: new Date('2024-09-09'),
    createdById: 1,
    createdBy: { id: 1, name: 'Admin', email: 'admin@example.com', role: 'Full Stack Developer' }
  },
  {
    id: 7,
    title: 'Security audit completion',
    description: 'Complete the comprehensive security audit and provide detailed report with recommendations.',
    status: TaskStatus.REVIEW,
    priority: TaskPriority.HIGH,
    type: TaskType.EPIC,
    estimatedHours: 12,
    actualHours: 15,
    progress: 85,
    dueDate: new Date('2024-09-09'), // 2 days overdue
    assignedToId: 10,
    assignedTo: { id: 10, name: 'Robert Martinez', email: 'robert@example.com', role: 'Security Specialist' },
    projectId: 5,
    project: { id: 5, name: 'Security Audit', status: 'ON_HOLD' },
    tags: ['security', 'audit', 'compliance'],
    createdAt: new Date('2024-08-25'),
    updatedAt: new Date('2024-09-10'),
    createdById: 1,
    createdBy: { id: 1, name: 'Admin', email: 'admin@example.com', role: 'Full Stack Developer' }
  },
  {
    id: 9,
    title: 'Mobile app performance optimization',
    description: 'Optimize mobile app performance by reducing load times and improving user experience metrics.',
    status: TaskStatus.IN_PROGRESS,
    priority: TaskPriority.MEDIUM,
    type: TaskType.IMPROVEMENT,
    estimatedHours: 8,
    actualHours: 10,
    progress: 40,
    dueDate: new Date('2024-09-06'), // 5 days overdue
    assignedToId: 6,
    assignedTo: { id: 6, name: 'Alex Chen', email: 'alex@example.com', role: 'Mobile Developer' },
    projectId: 3,
    project: { id: 3, name: 'Mobile App Development', status: 'ACTIVE' },
    tags: ['mobile', 'performance', 'optimization'],
    createdAt: new Date('2024-08-28'),
    updatedAt: new Date('2024-09-10'),
    createdById: 6,
    createdBy: { id: 6, name: 'Alex Chen', email: 'alex@example.com', role: 'Mobile Developer' }
  },
  {
    id: 11,
    title: 'User feedback implementation',
    description: 'Implement user feedback features including ratings, reviews, and suggestion collection system.',
    status: TaskStatus.TODO,
    priority: TaskPriority.LOW,
    type: TaskType.FEATURE,
    estimatedHours: 15,
    actualHours: 0,
    progress: 0,
    dueDate: new Date('2024-09-10'), // 1 day overdue
    assignedToId: 2,
    assignedTo: { id: 2, name: 'John Doe', email: 'john@example.com', role: 'Lead Developer' },
    projectId: 1,
    project: { id: 1, name: 'TaskMaster App', status: 'ACTIVE' },
    tags: ['user-feedback', 'ratings', 'reviews'],
    createdAt: new Date('2024-08-20'),
    updatedAt: new Date('2024-09-05'),
    createdById: 1,
    createdBy: { id: 1, name: 'Admin', email: 'admin@example.com', role: 'Full Stack Developer' }
  }
]

export default function OverduePage() {
  const [searchQuery, setSearchQuery] = React.useState('')
  const [statusFilter, setStatusFilter] = React.useState<string>('all')
  const [priorityFilter, setPriorityFilter] = React.useState<string>('all')
  const [sortBy, setSortBy] = React.useState<string>('overdue-desc') // Most overdue first
  const [tasks, setTasks] = React.useState(mockTasks)

  const filteredAndSortedTasks = React.useMemo(() => {
    let filtered = tasks.filter(task => {
      const matchesSearch = task.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                           task.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
                           task.tags.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase()))
      
      const matchesStatus = statusFilter === 'all' || task.status === statusFilter
      const matchesPriority = priorityFilter === 'all' || task.priority === priorityFilter
      
      return matchesSearch && matchesStatus && matchesPriority
    })

    // Sort tasks
    filtered.sort((a, b) => {
      const now = new Date()
      const aDaysOverdue = a.dueDate ? Math.floor((now.getTime() - a.dueDate.getTime()) / (1000 * 60 * 60 * 24)) : 0
      const bDaysOverdue = b.dueDate ? Math.floor((now.getTime() - b.dueDate.getTime()) / (1000 * 60 * 60 * 24)) : 0

      switch (sortBy) {
        case 'overdue-desc':
          return bDaysOverdue - aDaysOverdue // Most overdue first
        case 'overdue-asc':
          return aDaysOverdue - bDaysOverdue // Least overdue first
        case 'priority':
          const priorityOrder = { CRITICAL: 4, HIGH: 3, MEDIUM: 2, LOW: 1 }
          return priorityOrder[b.priority as keyof typeof priorityOrder] - priorityOrder[a.priority as keyof typeof priorityOrder]
        default:
          return 0
      }
    })

    return filtered
  }, [tasks, searchQuery, statusFilter, priorityFilter, sortBy])

  const getDaysOverdue = (dueDate: Date) => {
    const now = new Date()
    const diffTime = now.getTime() - dueDate.getTime()
    return Math.floor(diffTime / (1000 * 60 * 60 * 24))
  }

  const getOverdueColor = (daysOverdue: number) => {
    if (daysOverdue >= 7) return 'text-error-600 bg-error-100'
    if (daysOverdue >= 3) return 'text-warning-600 bg-warning-100'
    return 'text-orange-600 bg-orange-100'
  }

  const getPriorityColor = (priority: TaskPriority) => {
    switch (priority) {
      case TaskPriority.CRITICAL:
        return 'bg-error-100 text-error-800 border-error-200'
      case TaskPriority.HIGH:
        return 'bg-warning-100 text-warning-800 border-warning-200'
      case TaskPriority.MEDIUM:
        return 'bg-primary-100 text-primary-800 border-primary-200'
      case TaskPriority.LOW:
        return 'bg-neutral-100 text-neutral-800 border-neutral-200'
    }
  }

  const getStatusColor = (status: TaskStatus) => {
    switch (status) {
      case TaskStatus.TODO:
        return 'bg-neutral-100 text-neutral-800 border-neutral-200'
      case TaskStatus.IN_PROGRESS:
        return 'bg-primary-100 text-primary-800 border-primary-200'
      case TaskStatus.REVIEW:
        return 'bg-warning-100 text-warning-800 border-warning-200'
      case TaskStatus.TESTING:
        return 'bg-info-100 text-info-800 border-info-200'
      case TaskStatus.DONE:
        return 'bg-success-100 text-success-800 border-success-200'
      case TaskStatus.BLOCKED:
        return 'bg-error-100 text-error-800 border-error-200'
    }
  }

  const getPriorityIcon = (priority: TaskPriority) => {
    switch (priority) {
      case TaskPriority.CRITICAL:
        return <Zap className="h-3 w-3 text-error-600" />
      case TaskPriority.HIGH:
        return <Flag className="h-3 w-3 text-error-600" />
      case TaskPriority.MEDIUM:
        return <Flag className="h-3 w-3 text-warning-600" />
      default:
        return <Flag className="h-3 w-3 text-neutral-400" />
    }
  }

  const stats = [
    {
      name: 'Total Overdue',
      value: tasks.length,
      color: 'text-error-600',
      icon: AlertTriangle
    },
    {
      name: 'Critical Priority',
      value: tasks.filter(t => t.priority === TaskPriority.CRITICAL).length,
      color: 'text-error-600',
      icon: Zap
    },
    {
      name: 'Blocked',
      value: tasks.filter(t => t.status === TaskStatus.BLOCKED).length,
      color: 'text-warning-600',
      icon: ExclamationTriangle
    },
    {
      name: '7+ Days Overdue',
      value: tasks.filter(t => t.dueDate && getDaysOverdue(t.dueDate) >= 7).length,
      color: 'text-error-600',
      icon: Clock
    }
  ]

  return (
    <Layout>
      <div className="space-y-6">
        {/* Header */}
        <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          <div>
            <h1 className="text-3xl font-bold text-neutral-900 flex items-center space-x-2">
              <AlertTriangle className="h-8 w-8 text-error-600" />
              <span>Overdue Tasks</span>
            </h1>
            <p className="mt-1 text-neutral-600">
              Tasks that have passed their due date and require immediate attention
            </p>
          </div>
          <Button>
            <Plus className="mr-2 h-4 w-4" />
            New Task
          </Button>
        </div>

        {/* Critical Alert */}
        {tasks.filter(t => t.priority === TaskPriority.CRITICAL).length > 0 && (
          <Card className="border-error-200 bg-error-50">
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                <Zap className="h-5 w-5 text-error-600" />
                <span className="font-medium text-error-800">
                  {tasks.filter(t => t.priority === TaskPriority.CRITICAL).length} critical tasks are overdue
                </span>
              </div>
            </CardContent>
          </Card>
        )}

        {/* Stats Cards */}
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => (
            <Card key={stat.name}>
              <CardContent className="p-6">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    <stat.icon className={`h-8 w-8 ${stat.color}`} />
                  </div>
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      <dt className="text-sm font-medium text-neutral-500 truncate">
                        {stat.name}
                      </dt>
                      <dd className="text-2xl font-semibold text-neutral-900">
                        {stat.value}
                      </dd>
                    </dl>
                  </div>
                </div>
              </CardContent>
            </Card>
          ))}
        </div>

        {/* Search, Filters, and Sort */}
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-5">
          <div className="sm:col-span-2">
            <Input
              placeholder="Search overdue tasks..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              leftIcon={<Search className="h-4 w-4" />}
            />
          </div>
          <div>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
            >
              <option value="all">All Status</option>
              <option value={TaskStatus.TODO}>To Do</option>
              <option value={TaskStatus.IN_PROGRESS}>In Progress</option>
              <option value={TaskStatus.REVIEW}>In Review</option>
              <option value={TaskStatus.TESTING}>Testing</option>
              <option value={TaskStatus.BLOCKED}>Blocked</option>
            </select>
          </div>
          <div>
            <select
              value={priorityFilter}
              onChange={(e) => setPriorityFilter(e.target.value)}
              className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
            >
              <option value="all">All Priorities</option>
              <option value={TaskPriority.CRITICAL}>Critical</option>
              <option value={TaskPriority.HIGH}>High</option>
              <option value={TaskPriority.MEDIUM}>Medium</option>
              <option value={TaskPriority.LOW}>Low</option>
            </select>
          </div>
          <div>
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
            >
              <option value="overdue-desc">Most Overdue First</option>
              <option value="overdue-asc">Least Overdue First</option>
              <option value="priority">Priority</option>
            </select>
          </div>
        </div>

        {/* Tasks List */}
        <div className="space-y-4">
          {filteredAndSortedTasks.map((task) => {
            const daysOverdue = task.dueDate ? getDaysOverdue(task.dueDate) : 0
            return (
              <Card key={task.id} className="hover:shadow-warm-lg transition-shadow border-l-4 border-l-error-500">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <Link 
                            href={`/tasks/${task.id}`}
                            className="block hover:text-primary-600 transition-colors"
                          >
                            <h3 className="text-lg font-semibold text-neutral-900 mb-2">
                              {task.title}
                            </h3>
                          </Link>
                          <div className="flex items-center space-x-2 mb-3">
                            <Badge className={`${getOverdueColor(daysOverdue)} px-2 py-1 text-xs font-medium`}>
                              <AlertTriangle className="h-3 w-3 mr-1" />
                              {daysOverdue} day{daysOverdue !== 1 ? 's' : ''} overdue
                            </Badge>
                            <Badge className={getPriorityColor(task.priority)} size="sm">
                              <div className="flex items-center space-x-1">
                                {getPriorityIcon(task.priority)}
                                <span>{task.priority}</span>
                              </div>
                            </Badge>
                            <Badge className={getStatusColor(task.status)} size="sm">
                              {task.status.replace('_', ' ')}
                            </Badge>
                            <Badge variant="outline" size="sm">
                              {task.type}
                            </Badge>
                          </div>
                        </div>
                        <Button variant="ghost" size="sm">
                          <MoreVertical className="h-4 w-4" />
                        </Button>
                      </div>

                      <p className="text-neutral-600 text-sm mb-4 line-clamp-2">
                        {task.description}
                      </p>

                      {/* Progress Bar */}
                      <div className="mb-4">
                        <div className="flex items-center justify-between text-sm mb-2">
                          <span className="text-neutral-600">Progress</span>
                          <span className="font-semibold text-neutral-900">{task.progress}%</span>
                        </div>
                        <div className="h-2 rounded-full bg-neutral-200">
                          <div
                            className="h-2 rounded-full bg-primary-600 transition-all duration-300"
                            style={{ width: `${task.progress}%` }}
                          />
                        </div>
                      </div>

                      {/* Task Meta Information */}
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center space-x-4">
                          {task.assignedTo && (
                            <div className="flex items-center space-x-2">
                              <div className="h-6 w-6 rounded-full bg-primary-100 flex items-center justify-center">
                                <span className="text-xs font-medium text-primary-700">
                                  {task.assignedTo.name.charAt(0)}
                                </span>
                              </div>
                              <span className="text-neutral-600">{task.assignedTo.name}</span>
                            </div>
                          )}
                          {task.project && (
                            <div className="flex items-center space-x-1">
                              <span className="text-neutral-400">•</span>
                              <span className="text-neutral-600">{task.project.name}</span>
                            </div>
                          )}
                          {task.estimatedHours && (
                            <div className="flex items-center space-x-1">
                              <Clock className="h-4 w-4 text-neutral-400" />
                              <span className="text-neutral-600">{task.estimatedHours}h estimated</span>
                            </div>
                          )}
                        </div>

                        <div className="flex items-center space-x-1 text-error-600">
                          <Calendar className="h-4 w-4" />
                          <span className="font-medium">
                            Due {task.dueDate && formatDate(task.dueDate, 'MMM dd')}
                          </span>
                        </div>
                      </div>

                      {/* Tags */}
                      {task.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-3">
                          {task.tags.map((tag) => (
                            <span
                              key={tag}
                              className="inline-flex items-center rounded-full bg-neutral-100 px-2 py-1 text-xs font-medium text-neutral-600"
                            >
                              #{tag}
                            </span>
                          ))}
                        </div>
                      )}
                    </div>
                  </div>
                </CardContent>
              </Card>
            )
          })}
        </div>

        {/* Empty State */}
        {filteredAndSortedTasks.length === 0 && (
          <Card className="p-12 text-center">
            <div className="mx-auto w-24 h-24 bg-neutral-100 rounded-full flex items-center justify-center mb-4">
              <CheckSquare className="h-12 w-12 text-neutral-400" />
            </div>
            <h3 className="text-lg font-medium text-neutral-900 mb-2">No overdue tasks found</h3>
            <p className="text-neutral-600 mb-6">
              {searchQuery || statusFilter !== 'all' || priorityFilter !== 'all'
                ? 'Try adjusting your search or filter criteria.'
                : 'Excellent! All tasks are up to date. Keep up the good work!'}
            </p>
            <Button>
              <Plus className="mr-2 h-4 w-4" />
              Create New Task
            </Button>
          </Card>
        )}
      </div>
    </Layout>
  )
}